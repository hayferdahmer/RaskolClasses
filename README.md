...
<p align="center">
  <img src="https://img.shields.io/badge/1.10.0-8b0000?style=flat-square&label=release&labelColor=0a0a0a"/>
...
Корона даёт классу титул (Рассветный клинок, Коготь Вальрадиса, …, Тень на Рассвете, Голос Раскола) и партикл-ауру, видимую союзникам и врагам.
...
| Чернокнижник | Фолиант Раскола (скрытый путь) | Скверна, событийная | INT | Дрейн, диспел, анти-хил, амплификация |
...
Доступен только Магу или Жрецу с уровнем персонажа 40+ через **Фолиант Раскола** (выдача: `/rc foliant give`). Переход необратим: до чтения игрок обязан отречься от спеки и сбросить таланты.

**Скверна** — мера открытости фолианта: +6 за нанесённый урон, +3 за полученный, +10 за смерть врага в радиусе 10, +12 за «Голод Скверны»; −4/с вне боя. На 75+ урон ×1.2 («Распахнутая страница»), на 100 — тик 1% maxHP/с себе («Переполнение»).

**Цена силы:** откат 6.66% от нанесённого урона (true-урон себе, кап 30% maxHP за каст, не ниже 1 HP); в аду весь кит ×6 (`nether-mult`).

| Слот | Способность | Суть |
|---|---|---|
| 1 | Чёрное Слово | маг-урон цели; 66.6% урона → HP |
...
Спеки (occult 40): **Чёрный Маг** (урон/диспел) · **Адский Канал** (контроль/выживаемость). Дерево талантов `occult`: 9 узлов, 21 очко, ульт «Последняя Страница». Инсталляция: **Круг Хулы** (зона урона и анти-хила, +3 Скверны/с владельцу).
...
| Уровень персонажа | Среднее топ-5 скиллов AuraSkills (11 деревьев), кап 60 |
| Инсталляции | Мины, варды, руна и круг хулы; TTL, лимиты 2/200, килл-кредит |
| Короны | Рассвет / Вальрадис: титулы (6 классов) и партикл-ауры |
| Фолиант Раскола | Скрытый переход Маг/Жрец → Чернокнижник: гейты, LP-миграция, грант occult |
...
| `/rc foliant give <ник>` | admin | Выдать Фолиант Раскола |
...
```
mvn -B clean package
cp target/raskol-classes-1.10.0.jar plugins/
restart

...
Лимит 2 активных, TTL 60 с, запрет у спавна и в клеймах Towny.
Триггеры по врагам (чужая корона) и мобам; зоны для союзников.
...
Конфиг `vfx.*` переопределяет дефолты без перекомпиляции; опечатка = тишина, не краш.
Трейлы снарядов: фаербол с пламенным трейлом + треск огня; стрелы охотника с искрами.
Фидбек проков пассивок: партикл + звук + actionbar-тег (если `vfx.proc-actionbar: true`).

...
### Reverted
- Попытка удаления ActiveEffectManager и SpecEffects отменена: аудит показал,
  что оба класса живые (BossBarService рисует бары по флагам ActiveEffectManager;
  SpecService/SpecListener используют атрибутные модификаторы спеков, precise shot,
  rage burst и frost slow из SpecEffects). Файлы и их инициализация в RaskolClasses
